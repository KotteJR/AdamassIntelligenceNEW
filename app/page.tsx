            ...
            {user ? (
              <>
                ...
                <div className="relative" ref={menuRef}>
                  <button onClick={() => setIsMenuOpen(v => !v)} className="h-9 w-9 rounded-full overflow-hidden ring-1 ring-slate-200 align-middle">
                  ...
                  {isMenuOpen && (
                    <div className={`absolute right-0 mt-2 w-40 rounded-xl border p-2 text-sm shadow-sm ${isDark ? 'theme-card theme-border' : 'border-slate-200 bg-white'}`}>
                      <div className={`px-2 py-1 font-medium truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{user.name || user.email}</div>
                      <button onClick={() => { setIsMenuOpen(false); signOut(); }} className={`w-full text-left px-2 py-1 rounded-lg ${isDark ? 'hover:bg-slate-700 text-slate-200' : 'hover:bg-slate-50'}`}>Sign out</button>
                    </div>
                  )}
                </div>
              </>
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-8">
            ...
            {allReports.map((r) => {
              ...
              return (
                <button
                  ...
                  className={`snap-start w-64 h-32 shrink-0 rounded-xl p-4 text-left relative overflow-hidden`}
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {/* Create new tile */}
            <button onClick={() => setIsModalOpen(true)} className={`flex h-32 flex-col items-center justify-center rounded-xl border border-dashed ${isDark ? 'theme-border theme-card theme-text-secondary' : 'border-slate-300 bg-white text-slate-600'} shadow-sm transition hover:opacity-80`}>
